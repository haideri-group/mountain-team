...
// All the heavy work — 5 queries + per-member mapping — lives inside
// unstable_cache. Two invalidation paths:
//
//   1. Tag-based: any sync that mutates members/boards/issues/deployments
//      calls revalidateTag("overview") to drop the cache immediately
//      (see src/lib/sync/* and src/app/api/webhooks/jira/route.ts).
//      This is the primary, real-time freshness mechanism.
//
//   2. Time-based: revalidate=30 is a safety net so even if a sync forgets
//      to invalidate, the page never serves data older than 30 seconds.
//
// Why this is the right shape: the overview is identical for every viewer,
// so a single global cache entry is correct (no per-user keys). Auth still
// runs per-request in the handler — the cache only holds the (non-user-
// specific) response data, never the auth result.
//
// Net effect on page-load: handler drops from ~130ms → <5ms on every cache
// hit, which is ~99% of requests. Cache misses (first request after each
// sync, or every 30s as fallback) still pay the ~130ms but no more than
// once per data change.
async function computeOverviewUncached() {
  // Fetch all members
...
export async function GET(request: Request) {
  try {
    ...
    // requests from IPs in the admin-managed allowlist. Auth runs every
    // request; the cache only holds the (non-user-specific) data below.
